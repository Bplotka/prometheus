...
func StreamChunkedReadResponses(
	...
	for ss.Next() {
		...
		for iter.Next() {
			...
			chks, samples, err := encodeChunks(iter, 120, maxChunksInFrame, sampleLimit-numSamples)
			if err != nil {
				return err
			}
			numSamples += samples
...
func encodeChunks(iter storage.SeriesIterator, maxSamplesPerChunk int, maxChunks int, allowedNumSamples int) ([]prompb.Chunk, int, error) {
	var (
		chkMint int64
	...
	for isNext {
		numSamples++
		if allowedNumSamples > 0 && numSamples > allowedNumSamples {
			return nil, numSamples, HTTPError{
				msg:    fmt.Sprintf("exceeded sample limit"),
				status: http.StatusBadRequest,
			}
		}
